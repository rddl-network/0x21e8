...
@pytest.mark.skip(reason="the secret is not automatically removed. that's why this test fails sometimes and is skipped.")
def test_machine_before_wallet_init():
    from datetime import datetime
    x = datetime.now()
    response1 = client.post(
        "/machine", headers= { "accept": "application/json", "Content-Type": "application/json"},
            json = {  "name": "mytest - "+str(x),
                "ticker": "yourtest",
                "amount": 10000,
                "precision": 0,
...
def test_seed_creation():
    ...
    response1= client.post("/seed?mnemonic_phrase=%22potato%20drop%20kidney%20coral%20toilet%20elite%20uncover%20keep%20vintage%20beach%20eyebrow%20ethics%22")
    assert response1.status_code == 200
    response1= client.post("/seed?mnemonic_phrase=%22argue%20open%20meat%20hammer%20attend%20high%20milk%20human%20much%20session%20seat%20beauty%20permit%20symbol%20panther%20night%20firm%20butter%20jewel%20defense%20obtain%20wife%20ill%20brain%22")
    assert response1.status_code == 200
    response1= client.post("/seed?mnemonic_phrase=%22argue%20open%20meat%20hammer%20attend%20high%20milk%20human%20much%20session%20seat%20beauty%20permit%20symbol%20panther%20night%20firm%20butter%20jewel%20defense%20obtain%20wife%20ill%22")
    assert response1.status_code == 420
    assert response1.json() == {'detail': "A mnemonic has to contain 12 or 24 words"}
