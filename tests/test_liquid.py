...
def test_register_asset():
    resp = register_asset(
        asset_id=asset_obj["asset_id"], contract=json.loads(asset_obj["contract"]), registration_endpoint=RDDL_ASSET_REG_ENDPOINT
    )
...
def test_string_composition():
    assert F'{{"asset_id": "{asset_obj_2["asset_id"]}","contract":{json.dumps(asset_obj_2["contract"])}}}' == working_json_obj_str